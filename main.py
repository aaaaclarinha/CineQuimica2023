...
numero_de_particulas = 45
tamanho_caixa = 200.
numero_de_pink_particles = 45
...
lista_pink_particles = inicializar_lista_aleatoria(numero_de_pink_particles, raio= 3, massa=2.2e-23, tamanho=200)
...
# Create a list of circles to store all particles
circles = [None] * (numero_de_particulas + numero_de_pink_particles)

# Loop to create circles for regular and pink particles
for i in range(numero_de_particulas + numero_de_pink_particles):
    if i < numero_de_particulas:
        # Regular particles
        color = '#9A32CD'  # Set the color for regular particles
        circle = plt.Circle((lista_particulas[i].solpos[0][0], lista_particulas[i].solpos[0][1]),
                            lista_particulas[i].raio, ec="black", color=color, lw=1.25, zorder=15)
    else:
        # Pink particles
        color = 'aquamarine'  # Set the color for pink particles
        circle = plt.Circle((lista_pink_particles[i - numero_de_particulas].solpos[0][0], lista_pink_particles[i - numero_de_particulas].solpos[0][1]),
                            lista_pink_particles[i - numero_de_particulas].raio, ec="black", color=color, lw=1.25, zorder=15)
    
    circles[i] = circle
    ax1.add_patch(circle)

# Calculate and store velocities for both types of particles
mod_vel = [lista_particulas[i].solvel_mag[0] for i in range(len(lista_particulas))]
mod_pink_vel = [lista_pink_particles[i].solvel_mag[0] for i in range(len(lista_pink_particles))]
